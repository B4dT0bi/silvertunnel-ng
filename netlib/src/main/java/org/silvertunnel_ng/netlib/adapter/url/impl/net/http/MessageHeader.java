...
public class MessageHeader
{
	...
	public MessageHeader(final InputStream is) throws java.io.IOException
	{
		parseHeader(is);
	}
	 ...
	 */
	public synchronized String findValue(final String key)
	{
		if (key == null)
		{
			for (int i = nkeys; --i >= 0;)
	 ...
	 * @return location (position) of the given key
	 */
	...
	class HeaderIterator implements Iterator<String>
	{
		...
		public HeaderIterator(final String k, final Object lock)
		{
			key = k;
			this.lock = lock;
		}
	 ...
	 * sequence.
	 */
	public Iterator<String> multiValueIterator(final String k)
	{
	 ...
	 */
	public synchronized void add(final String k, final String v)
	{
		grow();
		keys[nkeys] = k;
		values[nkeys] = v;
	 ...
	 * allowed
	 */
	public synchronized void prepend(final String k, final String v)
	{
		grow();
	...
	private void grow()
	{
		...
		{
			final String[] nk = new String[nkeys + 4];
			final String[] nv = new String[nkeys + 4];
			if (keys != null)
			{
	...
	public synchronized void setIfNotSet(final String key, final String value)
	{
	...
	public void parseHeader(final InputStream is) throws java.io.IOException
	{
		synchronized (this)
		{
			nkeys = 0;
	...
	public void mergeHeader(final InputStream is) throws java.io.IOException
	{
		if (is == null)
		{
			return;
		...
		while (firstc != '\n' && firstc != '\r' && firstc >= 0)
		{
			...
			parseloop:
			{
				while ((c = is.read()) >= 0)
				{
					...
					if (len >= s.length)
					{
						final char[] ns = new char[s.length * 2];
