...
class BasicAuthentication extends AuthenticationInfo
{
	...
	/**
	 * The authentication string for this host, port, and realm. This is a
	 * simple BASE64 encoding of "login:password".
	 */
	String auth;
			...
			String realm, PasswordAuthentication pw)
	{
		...
		final String plain = pw.getUserName() + ":";
		byte[] nameBytes = null;
		try
		{
		...
		System.arraycopy(passwdBytes, 0, concat, nameBytes.length,
				passwdBytes.length);
		this.auth = "Basic " + DatatypeConverter.printBase64Binary(concat);
		this.pw = pw;
	}
	 ...
	 * Create a BasicAuthentication.
	 */
	public BasicAuthentication(final boolean isProxy, final String host,
			final int port, final String realm, final String auth)
	{
		super(isProxy ? PROXY_AUTHENTICATION : SERVER_AUTHENTICATION,
				BASIC_AUTH, host, port, realm);
	 ...
	 * Create a BasicAuthentication.
	 */
	public BasicAuthentication(final boolean isProxy, final URL url,
			final String realm, final PasswordAuthentication pw)
	{
		super(isProxy ? PROXY_AUTHENTICATION : SERVER_AUTHENTICATION,
	 ...
	 */
	public BasicAuthentication(final boolean isProxy, final URL url,
			final String realm, final String auth)
	{
		super(isProxy ? PROXY_AUTHENTICATION : SERVER_AUTHENTICATION,
				BASIC_AUTH, url, realm);
		this.auth = "Basic " + auth;
	 ...
	 * @param raw
	 *            The raw header values for this connection, if needed.
	 * @return true if all goes well, false if no headers were set.
	 */
	@Override
	boolean setHeaders(final HttpURLConnection conn, final HeaderParser p,
			final String raw)
	{
	...
	/**
	 * For Basic Authentication, the security parameters can never be stale. In
	 * other words there is no possibility to reuse the credentials. They are
	 * always either valid or invalid.
	 */
	@Override
	...
	/**
	 * @return the common root path between npath and path. This is used to
	 *         detect when we have an authentication for two paths and the root
	 *         of th authentication space is the common root.
	 */
