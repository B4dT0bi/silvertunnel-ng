...
public class AuthorityServers {
     ...
     * const char *dirservers[] = { "moria1 orport=9101 no-v2 "
     ...
     * "154.35.32.5:80 CF6D 0AAF B385 BE71 B8E1 11FC 5CFF 4B47 9237 33BC", NULL
     * }; for (i=0; dirservers[i]; i++) { if
     * (parse_dir_server_line(dirservers[i], type, 0)<0) { log_err(LD_BUG,
    ...
    private static Collection<Router> parseAuthorityRouters() {
        ...
        for (final String singleRawData : RAW_DATA) {
            // one server
            try {
                final Matcher m = pattern.matcher(singleRawData);
                if (m.find()) {
                    // extract single values
                    final String nickname = m.group(1);
