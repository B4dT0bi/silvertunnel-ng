...
public class CellRelayEstablishRendezvous extends CellRelay
{
			...
			throws TorException
	{
		...
		// check whether the cookie size suits into data and is at least 20
		// bytes
		if (cookie.length < 20)
		{
			throw new TorException(
					"CellRelayEstablishRendezvous: rendevouz-cookie is too small");
		}
		if (cookie.length > data.length)
		{
			throw new TorException(
					"CellRelayEstablishRendezvous: rendevouz-cookie is too large");
		}
