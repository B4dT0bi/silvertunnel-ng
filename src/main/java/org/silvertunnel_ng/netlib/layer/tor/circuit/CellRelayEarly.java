 ...
 * @author Tobias Boese
 */
public class CellRelayEarly extends Cell // TODO : try to extend CellRelay instead and just overwrite the needed stuff
{
	/** */
	...
	{
		super(s.getCircuit(), Cell.CELL_RELAY_EARLY);
	 ...
	 */
	CellRelayEarly(final byte[] data) throws TorException
	{
		super(data);
		initFromData();
	}
	 ...
	 */
	CellRelayEarly(final Circuit circ, final Cell cell) throws TorException
	{
	 ...
	 */
	CellRelayEarly(final InputStream in) throws IOException, TorException
	{
		super(in);
		initFromData();
	}
	...
	void initFromData() throws TorException
	{
				...
				.getRouteEstablished(); ++encryptingRouter)
		{
			...
			if (LOG.isDebugEnabled())
			{
				LOG.info("CellRelayEarly.initFromDate with encryptingRouter="
			...
			if ((payload[CellRelayEarly.RELAY_RECOGNIZED_POS] == 0)
					&& (payload[CellRelayEarly.RELAY_RECOGNIZED_POS + 1] == 0))
			{
				// check digest.

				// save digest
				System.arraycopy(payload, CellRelayEarly.RELAY_DIGEST_POS, digest,
						0, CellRelayEarly.RELAY_DIGEST_SIZE);
				// set to ZERO
				payload[CellRelayEarly.RELAY_DIGEST_POS] = 0;
				payload[CellRelayEarly.RELAY_DIGEST_POS + 1] = 0;
				payload[CellRelayEarly.RELAY_DIGEST_POS + 2] = 0;
				payload[CellRelayEarly.RELAY_DIGEST_POS + 3] = 0;
				// calculate digest
				...
				System.arraycopy(digest, 0, payload,
						CellRelayEarly.RELAY_DIGEST_POS, CellRelayEarly.RELAY_DIGEST_SIZE);
				// check digest
						...
						&& (digest[3] == digestCalc[3]))
				{
					if (LOG.isDebugEnabled())
					{
						LOG.debug("CellRelayEarly.initFromData(): backward digest from "
		...
		if (!digestVerified)
		{
			LOG.warn("CellRelayEarly.initFromData(): Received "
					+ Encoding.toHexString(digest)
		...
		relayCommand = payload[CellRelayEarly.RELAY_COMMAND_POS];
		streamId = Encoding.byteArrayToInt(payload,
				CellRelayEarly.RELAY_STREAMID_POS, CellRelayEarly.RELAY_STREAMID_SIZE);
		length = Encoding.byteArrayToInt(payload, CellRelayEarly.RELAY_LENGTH_POS,
				CellRelayEarly.RELAY_LENGTH_SIZE);
		System.arraycopy(payload, CellRelayEarly.RELAY_DATA_POS, data, 0,
				CellRelayEarly.RELAY_DATA_SIZE);

		if (LOG.isDebugEnabled())
		{
			LOG.debug("CellRelayEarly.initFromData(): " + toString());
		}
	}
	...
	byte[] toByteArray()
	{
		if (LOG.isDebugEnabled())
		{
			LOG.debug("CellRelayEarly.toByteArray() for "
					+ outCircuit.getRouteEstablished() + " layers");
		}
		// put everything in payload
		payload[CellRelayEarly.RELAY_COMMAND_POS] = relayCommand;
		System.arraycopy(Encoding.intToNByteArray(streamId,
				CellRelayEarly.RELAY_STREAMID_SIZE), 0, payload,
				CellRelayEarly.RELAY_STREAMID_POS, CellRelayEarly.RELAY_STREAMID_SIZE);
		System.arraycopy(
				Encoding.intToNByteArray(length, CellRelayEarly.RELAY_LENGTH_SIZE),
				0, payload, CellRelayEarly.RELAY_LENGTH_POS,
				CellRelayEarly.RELAY_LENGTH_SIZE);
		System.arraycopy(data, 0, payload, CellRelayEarly.RELAY_DATA_POS,
				CellRelayEarly.RELAY_DATA_SIZE);
		...
		System.arraycopy(digest, 0, payload, CellRelayEarly.RELAY_DIGEST_POS,
				CellRelayEarly.RELAY_DIGEST_SIZE);

		if (LOG.isDebugEnabled())
		{
			LOG.debug("CellRelayEarly.toByteArray(): " + toString());
		}
		...
		for (int i = i0; i >= 0; --i)
		{
			if (LOG.isDebugEnabled())
			{
				LOG.debug("CellRelayEarly.toByteArray with encryptingRouter=" + i
	...
	public String toString()
	{
		...
		// main header
		sb.append("RelayEarly cell for circuit " + getCircuitId() + "/stream "
