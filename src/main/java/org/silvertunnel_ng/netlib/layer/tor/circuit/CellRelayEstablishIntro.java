...
public class CellRelayEstablishIntro extends CellRelay
{
	public CellRelayEstablishIntro(Circuit c, HiddenServiceProperties service)
	{
		...
		System.arraycopy(c.getRouteNodes()[c.getRouteNodes().length - 1]
				.getKeyHandshake(), 0, hsInput, 0, 20);
		System.arraycopy("INTRODUCE".getBytes(), 0, hsInput, 20, 9);
		final byte[] hs = Encryption.getDigest(hsInput);
		// concat all data
		final byte[] pk = Encryption.getPKCS1EncodingFromRSAPublicKey(service
				.getPublicKey());
		...
		// signature
		final byte[] signature = Encryption.signData(input,
