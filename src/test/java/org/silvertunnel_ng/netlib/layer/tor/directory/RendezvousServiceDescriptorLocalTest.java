...
public class RendezvousServiceDescriptorLocalTest
{
	...
	private static final Date EXAMPLE_SD_VALID_DATE = Util
			.parseUtcTimestamp("2010-03-09 17:00:00");
	private static final Date EXAMPLE_SD_INVALID_DATE = Util
			.parseUtcTimestamp("2010-03-25 00:30:00");
	private static final Date EXAMPLE2_SD_VALID_DATE = Util
			.parseUtcTimestamp("2011-02-14 17:25:44");
	...
	public void testGetRendezvousTimePeriod()
	{
		...
		final Date now = new Date(1188241957L * 1000L);
		final int result = RendezvousServiceDescriptorUtil
				.getRendezvousTimePeriod(rendezvousDescriptorServiceId, now);
		assertEquals("wrong result of getRendezvousTimePeriod()", 0x000035B9,
				result);
	}
	...
	{
		final byte[] rendezvousDescriptorServiceId = Encoding
				.parseBase32("duskgytldkxiuqc6");
		final Date now = new Date(0L * 1000L);
		final int result = RendezvousServiceDescriptorUtil
				.getRendezvousTimePeriod(rendezvousDescriptorServiceId, now);
	...
	public void testGetRendezvousDescriptorIdBase32()
	{
		...
		final int replica = 0;
		final Date now = Util.parseUtcTimestamp("2010-03-02 22:00:00");
		final String result = RendezvousServiceDescriptorUtil
				.getRendezvousDescriptorIdBase32(
						hiddenServicePermanentIdBase32, replica, now);
		assertEquals("wrong result of getRendezvousDescriptorIdBase32()",
				"4gyqu7zqgzn2dkfdyirq2ire4jgnzb24", result);
	}
	...
	public void testGetRendezvousDescriptorIdBase32_2()
	{
		...
		final int replica = 0;
		final Date now = new Date(0);
		final String result = RendezvousServiceDescriptorUtil
				.getRendezvousDescriptorIdBase32(
						rendezvousDescriptorServiceIdBase32, replica, now);
		assertEquals("wrong result of getRendezvousDescriptorIdBase32()",
				"txbvqnxzzhy45dbxkjiv4ll56ry7sild", result);
	}
	...
	public void testGetRendezvousDescriptorIdBase32_3()
	{
		...
		final String result = RendezvousServiceDescriptorUtil
				.getRendezvousDescriptorIdBase32(
						rendezvousDescriptorServiceIdBase32, replica, now);
		assertEquals("wrong result of getRendezvousDescriptorIdBase32()",
				"fxvidli3evifukggw66byny7rnwtitdo", result);
	}
	...
	public void testParseValidRendezvousServiceDescriptor() throws Exception
	{
		...
		final String rendezvousServiceDescriptorStr = FileUtil.getInstance()
				.readFileFromClasspath(
						EXAMPLE_RENDEZVOUS_SERVICE_DESCRIPTOR_PATH);
		final RendezvousServiceDescriptor sd = new RendezvousServiceDescriptor(
				rendezvousServiceDescriptorStr, EXAMPLE_SD_VALID_DATE);

		// check basics
		assertEquals("wrong version", "2", sd.getVersion());
		assertEquals("wrong publication-time",
				Util.parseUtcTimestamp("2010-03-09 13:41:53"),
				sd.getPublicationTime());
	...
	@Test
	public void testParseValidRendezvousServiceDescriptorExample2()
			throws Exception
	{
		// read and parse
		final String rendezvousServiceDescriptorStr = FileUtil.getInstance()
				.readFileFromClasspath(
						EXAMPLE2_RENDEZVOUS_SERVICE_DESCRIPTOR_PATH);
		final RendezvousServiceDescriptor sd = new RendezvousServiceDescriptor(
				rendezvousServiceDescriptorStr, EXAMPLE2_SD_VALID_DATE);
		LOG.info("sd=" + sd);

		// check basics
		assertEquals("wrong version", "2", sd.getVersion());
		assertEquals("wrong publication-time", EXAMPLE2_SD_VALID_DATE,
				sd.getPublicationTime());
		...
		final Iterator<SDIntroductionPoint> ipsi = ips.iterator();
		assertEquals("introduction point 1 address", new TcpipNetAddress(
				"141.161.20.51:9001"), ipsi.next().getIpAddressAndOnionPort());
	}

	@Test
	public void testParseOutdatedRendezvousServiceDescriptor()
			throws IOException
	{
		// read and parse
		final String rendezvousServiceDescriptorStr = FileUtil.getInstance()
		...
		{
			final RendezvousServiceDescriptor sd = new RendezvousServiceDescriptor(
					rendezvousServiceDescriptorStr, EXAMPLE_SD_INVALID_DATE);
	...
	@Test
	public void testParseInvalidRendezvousServiceDescriptor()
			throws IOException
	{
		// read and parse
		final String rendezvousServiceDescriptorStr = FileUtil.getInstance()
				.readFileFromClasspath(
						EXAMPLE_RENDEZVOUS_SERVICE_DESCRIPTOR_INVALID_PATH);
		try
		{
			final RendezvousServiceDescriptor sd = new RendezvousServiceDescriptor(
					rendezvousServiceDescriptorStr, EXAMPLE_SD_VALID_DATE);
	...
	public void testFormatRendezvousServiceDescriptor() throws Exception
	{
		...
		// check formatting
		final String reformattedRendezvousServiceDescriptorStr = sd
				.toServiceDescriptorString();
		...
		final String expextedRendezvousServiceDescriptorStr = Pattern
				...
				.replaceAll(
						"-----BEGIN SIGNATURE-----\n-----END SIGNATURE-----");
		LOG.info("expextedRendezvousServiceDescriptorStr="
	...
	public void testReparsedRendezvousServiceDescriptor() throws Exception
	{
		...
		final RendezvousServiceDescriptor sdReparsed = new RendezvousServiceDescriptor(
				new String(sdBytes, Util.UTF8), EXAMPLE_SD_VALID_DATE, false);
		assertEquals("wrong reformatted RendezvousServiceDescriptorStr",
				sd.toString(), sdReparsed.toString());
	}
}
